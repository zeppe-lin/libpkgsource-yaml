...
void parses_declarations_with_exact_provenance()
{
  ...
  require_equal(declarations.size(), std::size_t{2},
                "expected two profile declarations");
  require_equal(declarations[0].name().name(), std::string("@toolchain"),
...
{
  pkgsource::profile_catalog first = pkgsource::profile_catalog::seal(
      pkgsource::yaml::parse_profiles_yaml(
          test_documents::profiles,
          pkgsource::source_origin("profiles.yml")));
  pkgsource::profile_catalog reordered = pkgsource::profile_catalog::seal(
      pkgsource::yaml::parse_profiles_yaml(
  ...
  require_equal(toolchain.expansion().size(), std::size_t{2},
  ...
  require_equal(toolchain.expansion()[1].package().name(), std::string("gcc"),
