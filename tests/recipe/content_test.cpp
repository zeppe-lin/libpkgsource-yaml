...
void parses_complete_recipe_declaration()
{
  ...
  require_equal(declaration.release().package().name(), std::string("example"),
                "package name must be parsed");
  require_equal(declaration.provenance().document(), std::string("recipe.yml"),
                "recipe provenance must retain the document label");
  require_equal(declaration.provenance().path(), std::string("document"),
...
void parsed_declaration_seals_complete_source_authority()
{
  ...
  pkgsource::source_snapshot snapshot = pkgsource::seal_source(
      pkgsource::source_origin("recipe.yml"),
      pkgsource::yaml::parse_recipe_yaml(
          test_documents::complete_recipe,
          pkgsource::source_origin("recipe.yml")),
  ...
  require_equal(recipe.release().version_release(), std::string("1.2.3-1"),
  ...
  require_equal(recipe.sources().size(), std::size_t{2},
                "both source inputs must be retained");
  require_equal(recipe.build_requirements().size(), std::size_t{3},
                "profile expansion and package build requirements must seal");
  require_equal(recipe.run_requirements().size(), std::size_t{1},
  ...
  require_equal(
      recipe.lifecycle_requirements(
          ...
          .size(),
      std::size_t{1}, "lifecycle requirements must seal");
  require(recipe.lifecycle(pkgsource::lifecycle_action::post_install) != nullptr,
...
{
  const pkgsource::profile_catalog catalog = profiles();
  pkgsource::source_snapshot first = pkgsource::seal_source(
      pkgsource::source_origin("recipe.yml"),
      pkgsource::yaml::parse_recipe_yaml(
          test_documents::complete_recipe,
          pkgsource::source_origin("recipe.yml")),
  ...
  pkgsource::source_snapshot reordered = pkgsource::seal_source(
      ...
      pkgsource::yaml::parse_recipe_yaml(
          test_documents::reordered_recipe,
          pkgsource::source_origin("other.yml")),
      catalog);

  require_equal(first.identity(), reordered.identity(),
...
int main()
{
  return test_support::run({
      {"parses complete recipe declaration", parses_complete_recipe_declaration},
